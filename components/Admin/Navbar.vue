...
<template>
  <nav class="flex flex-grow justify-end items-center">
    ...
    <div class=" overflow-ellipsis"><span class="font-semibold">{{ data?.model?.name }}</span></div>
    <div test-id="profile-avatar" v-if="userStore.user" class="dropdown dropdown-end">
      <div tabindex="0" role="button" class="btn btn-ghost btn-circle avatar">
        <div class="w-10 h-10 rounded-full">
          <nuxt-img
              :src="data?.model.avatar !== '' ? user.model.avatar :  `https://api.dicebear.com/9.x/pixel-art/svg?seed=${data?.model.name}`"
              alt="Profile picture"
