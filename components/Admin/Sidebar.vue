...
<template>
  <aside test-id="admin-sidebar" class="rounded-xl shadow-lg min-h-[90vh] h-full w-[20vw] bg-gray-50 dark:bg-[#111924]">
    ...
    <div class="text-sm text-regular px-6 mt-2 ">Tumbuh dan berkembang bersama kami.</div>
    <div v-if="userMenu.length" class="mt-10">
      <nuxt-link v-for="item in userMenu"
                 ...
                 class="transition-all flex items-center rounded-tl-sm rounded-bl-sm hover:border-primary px-4 hover:border-solid hover:text-gray-100 hover:border-l-4 dark:border-[#2a3545] pl-4 mb-2">
        <div class="flex gap-4 items-center dark:hover:bg-[#2a3545] hover:bg-primary py-2 w-full pl-2 rounded"
          ...
          <span class="hover:font-bold">{{ item.title }}</span>
        </div>
      </nuxt-link>
    </div>
  </aside>
